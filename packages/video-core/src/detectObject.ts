...
export function buildForegroundMask(
  ...
  // Compare squared distance to squared threshold — avoids sqrt entirely
  ...
  for (let i = 0; i < n; i++) {
    ...
    mask[i] = (dr * dr + dg * dg + db * db) > thresh2 ? 1 : 0;
  }
...
export function detectObjectFast(
  ...
  return { mask: closed, centroid: { x: sumX / cnt, y: sumY / cnt } };
}
